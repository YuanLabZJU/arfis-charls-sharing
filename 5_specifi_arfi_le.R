...
IR_adjust <- IR_adjust %>% 
  ...
  mutate(IR_adjust = val*HR_est/Prev_HR_Sum)


life_table <- data.frame(age = rep(c(50:99),2),
                         age_id = rep(c(rep(15,5), rep(16,5), rep(17,5), rep(18,5), 
                                        rep(19,5), rep(20,5), rep(30,5), rep(31,5), 
                                        rep(32,5), rep(235,5)), 2),
                         Exposure = c(rep(0,50), rep(1,50))) %>% 
  left_join(IR_adjust, by = c("age_id", "Exposure")) %>% 
  group_by(Exposure) %>%
  mutate(
    Prob_Surv = 1-IR_adjust/100000,
    # Prob_Surv = 1,
